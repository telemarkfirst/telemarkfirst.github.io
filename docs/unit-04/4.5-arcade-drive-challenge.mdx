...
**Task:** Finish the two-motor arcade drive. Apply deadzones, toggle precision mode only on the A-button press edge, mix forward and turn into separate left and right powers, limit both with the right trigger, and send the final values to the motors and telemetry.
